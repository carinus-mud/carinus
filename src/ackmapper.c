...
char *string_format(char *str, int *numlines, int width, int height, bool unjus
t)
{
         ...
         sp = break_arg(sp, arg, sizeof(arg), width, &len, &alen))
    {
        ...
        if (blen+1>=width || iseol(*arg))
        {
            ...
            *pbuf='\0';
            ...
            {
                strcpy(pret, string_justify(buf, blen-alen, width, numwords, &j
len));
                pret += jlen;
            }
            pbuf=buf;
        ...
        if (blen+1>=width || iseol(*arg))
        {
            ...
            *pbuf='\0';
            ...
            if (currline<MAP_Y)
                ...
                {
                    strcpy(pret, string_justify(buf, blen-alen, width, numwords
, &jlen));
                    pret += jlen;
                }
                if (currline==MAP_Y)
...
void disp_map(char *border, char *map, CHAR_DATA *ch)
{
    ...
    strcat(disp, map_format(exit_string(ch, ch->in_room), y, bufs, &y, cols, ro
ws, TRUE));
             ...
             int x, int y, int min, int max, int line_of_sight )
{
    ...
    for ( looper = 0, victim = room->first_person; victim; victim = victim->nex
t_in_room )
...
//                 ch->pcdata->learned[gsn_find_doors] > number_percent() &&
                 !str_cmp( door->keyword, "" ) )  )  )
        {
            map[x+door_marks[door->vdir][0]][y+door_marks[door->vdir][1]]=door_
type;
            if (  door_type < DOOR_CLOSED &&
                  ( line_of_sight == LOS_INITIAL ||
                    door->vdir == line_of_sight ) &&
                    map[x+offsets[door->vdir][0]][y+offsets[door->vdir][1]]==SE
CT_UNSEEN  )
            {
...
void do_mapper( CHAR_DATA *ch, const char *argument )
{
