...
void show_char_to_char_0( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   {
      if( !victim->switched )
         send_to_char_color( "&R[Link Dead] ", ch );
...
{
    mudstrlcat( buf, victim->pcdata->title, MAX_STRING_LENGTH );
    mudstrlcat( buf, victim->pcdata->cpose, MAX_STRING_LENGTH );
}
   ...
   else
   {
      switch ( victim->position )
      {
         case POS_DEAD:
            ...
            break;
         case POS_MORTAL:
            mudstrlcat( buf, " (is wounded)", MAX_STRING_LENGTH );
            break;
         case POS_INCAP:
            mudstrlcat( buf, " (is incapacitated)", MAX_STRING_LENGTH );
            break;
         case POS_STUNNED:
            mudstrlcat( buf, " (is stunned)", MAX_STRING_LENGTH );
            break;
         case POS_SLEEPING:
               mudstrlcat( buf, " (is sleeping)", MAX_STRING_LENGTH );
            break;
         case POS_RESTING:
               mudstrlcat( buf, " (is resting)", MAX_STRING_LENGTH );
            break;
         case POS_SITTING:
               mudstrlcat( buf, " (is sitting)", MAX_STRING_LENGTH );
            break;
         case POS_STANDING:
            if( IS_IMMORTAL( victim ) )
               mudstrlcat( buf, " (is glowing)", MAX_STRING_LENGTH );
            else
               if( ( victim->in_room->sector_type == SECT_UNDERWATER )
                   && !IS_AFFECTED( victim, AFF_AQUA_BREATH ) && !IS_NPC( victim ) )
               mudstrlcat( buf, " (is drowning)", MAX_STRING_LENGTH );
            else if( victim->in_room->sector_type == SECT_UNDERWATER )
               mudstrlcat( buf, " (is swimming)", MAX_STRING_LENGTH );
            else
               if( ( victim->in_room->sector_type == SECT_OCEANFLOOR )
                   && !IS_AFFECTED( victim, AFF_AQUA_BREATH ) && !IS_NPC( victim ) )
               mudstrlcat( buf, " (is drowning)", MAX_STRING_LENGTH );
            else if( victim->in_room->sector_type == SECT_OCEANFLOOR )
               mudstrlcat( buf, " (is standing)", MAX_STRING_LENGTH );
            else if( IS_AFFECTED( victim, AFF_FLOATING ) || IS_AFFECTED( victim, AFF_FLYING ) )
               mudstrlcat( buf, " (is hovering)", MAX_STRING_LENGTH );
            else
               mudstrlcat( buf, " (is standing)", MAX_STRING_LENGTH );
            break;
         case POS_MOUNTED:
            mudstrlcat( buf, " (is mounted)", MAX_STRING_LENGTH );
            break;
         case POS_FIGHTING:
         case POS_EVASIVE:
         case POS_DEFENSIVE:
         case POS_AGGRESSIVE:
         case POS_BERSERK:
            mudstrlcat( buf, " (is fighting)", MAX_STRING_LENGTH );
            break;
      }
   }
...
*/
      ...
      if( xIS_SET( wch->act, PLR_WIZINVIS ) )
         ...
         invis_str[0] = '\0';
      snprintf( buf, MAX_STRING_LENGTH, "%*s%s%s%s%s%s%s&w%-10s &G[%-15s] &c%s.%s%s%s\r\n",
                ...
                char_name, wch->pcdata->customclass, wch->pcdata->title, extra_title, clan_name, council_name );
   ...
   if( first_deadly )
{
   ...
   if ( !ch )
    ...
    ch_printf( ch, "\n\r&WYou see %d player%s in the game.&D\n\r", nMatch, nMatch == 1 ? "" : "s" );
    return;
}
...
void do_commands( CHAR_DATA* ch, const char* argument)
{
   ...
   set_pager_color( AT_PLAIN, ch );
	...
	if( !str_cmp( arg, "communicate" ) || !str_cmp( arg, "comm") )
		{
...
void do_whois( CHAR_DATA* ch, const char* argument)
{
   ...
   if( IS_IMMORTAL( ch ) )
   {
      ...
      if( xIS_SET( victim->act, PLR_SILENCE ) || xIS_SET( victim->act, PLR_FREEZE ) || xIS_SET( victim->act, PLR_NO_EMOTE )
         ...
         || IS_SET( victim->pcdata->flags, PCFLAG_NODESC ) || IS_SET( victim->pcdata->flags, PCFLAG_NOBIO ) || IS_SET( victim->pcdata->flags, PCFLAG_NOCPOSE ) )
      {
         ...
         if( IS_SET( victim->pcdata->flags, PCFLAG_NOCPOSE ) )
            mudstrlcat( buf2, " nocpose", MAX_STRING_LENGTH );
         mudstrlcat( buf2, ".\r\n", MAX_STRING_LENGTH );
