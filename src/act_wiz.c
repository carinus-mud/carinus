...
//   VARIABLE_DATA *vd;
  ...
  if(!IS_NPC(victim))
	...
	pager_printf_color( ch,"&c|  &cHost:&W%-14s  &cRecent IP:&W%-14s &cPrevious IP:&w%-14s   &c|\r\n",
			victim->desc->host,
			victim->pcdata->recent_site ? victim->pcdata->recent_site : "Unknown", 
			victim->pcdata->prev_site ? victim->pcdata->prev_site : "Unknown" );
        pager_printf_color( ch, "&c| &cStatus: &w%-11s   &cAuth By: &W%-30s               &c|\r\n", 
			CAN_PKILL( victim ) ? "Deadly" :
                        IS_PKILL( victim ) ? "Pre-Deadly" : "Non-Deadly",
			victim->pcdata->authed_by[0] != '\0' ? victim->pcdata->authed_by : "(unknown)");
	pager_printf_color( ch,"&c+==================+===========+=====================+========================+\r\n");
	pager_printf_color( ch,"&c|                  &c| &cStr: &W%-3d  &c|  &cClass: &W%-10s  &c|  &cWimpy : &W%-2d            &c|\r\n",
			  get_curr_str( victim ), IS_NPC( victim ) ? victim->Class < MAX_NPC_CLASS && victim->Class >= 0 ?
	...
	pager_printf_color( ch,"&c|                  &c| &cCha: &W%-3d  &c|  &cMove : &W%-5d/%-5d &c|                        &c|\r\n",
			get_curr_cha(victim), victim->move, victim->max_move);
	pager_printf_color( ch,"&c|                  &c| &cLck: &W%-3d  &c|  &cAC   : &W%-5d       &c|  &cSecs  : &W%-4d        &c|\r\n",
			get_curr_lck(victim), GET_AC( victim ), ( int )victim->played);
	pager_printf_color( ch,"&c|                  &c| &cHit: &W%-3d  &c|  &cAlign: &W%-5d       &c|  &cTimer : &W%-4d          &c|\r\n",
			GET_HITROLL( victim ), victim->alignment, victim->timer); 
	pager_printf_color( ch,"&c|  &cPos'n : &W%-3d     &c| &cDam: &W%-3d  &c|  &cWimpy: &W%-5d       &c|  &cGold  : &W%-8d      &c|\r\n",
			victim->position, GET_DAMROLL( victim ), victim->wimpy, victim->gold);

	pager_printf_color( ch,"&c+==================+===========+=====================+========================+\r\n");
	pager_printf_color( ch,"&c|    &cFighting: &W%-10s     &cMaster : &W%-10s      &cLeader : &W%-10s    &c|\r\n",
                       victim->fighting ? victim->fighting->who->name : "(none)",
                       victim->master ? victim->master->name : "(none)", victim->leader ? victim->leader->name : "(none)" );
	pager_printf_color( ch,"&c+=============================================================================+\r\n");
	pager_printf_color( ch,"&c|    &cSpeaks: &W%-3d              &cSpeaking: &W%-5d          &cExp: &w%-15d   &c|\r\n",
                       victim->speaks, victim->speaking, victim->exp);
	pager_printf_color( ch,"&c+=============================================================================+\r\n");
	pager_printf_color(ch, "&c| &cSave Vs Poison: &W%-2d  &cVs  Wand: &W%-2d &cVs  Para: &W%-2d &cVs  Breath: &W%-2d &cVs  Spell: &W%-2d  &c|\r\n",
                       victim->saving_poison_death,
                       victim->saving_wand,
                       victim->saving_para_petri,
                       victim->saving_breath,
                       victim->saving_spell_staff );
      pager_printf_color( ch, "&c|           Thirst: &w%-3d    &cFull: &w-3%d    &cDrunk: &w%-3d    &cBleeding: &w%-3d          &c|\r\n",
                          victim->pcdata->condition[COND_THIRST],
                          victim->pcdata->condition[COND_FULL], victim->pcdata->condition[COND_DRUNK],
			  victim->pcdata->condition[COND_BLEEDING] );
	pager_printf_color( ch,"&c+=============================================================================+\r\n");
	   if( victim->pcdata->clan )
		      pager_printf_color( ch, "&c|   &c%-6s: &w%-60s      &c|\r\n",
	...
	pager_printf_color( ch, "Affected by: &C%-40s \r\n", affect_bit_name( &victim->affected_by ) );

		pager_printf_color( ch,"&cLanguages: &W"); 
  ...
  if(IS_NPC(victim))
    {
	pager_printf_color( ch,"\r\n&c+=============================================================================+\r\n");
	pager_printf_color( ch,"&c|  &cName: &W%-63s      &c|\r\n", victim->name );
	pager_printf_color( ch,"&c+=======================+===========+=====================+===================+\r\n");
	pager_printf_color( ch,"&c|  &cVnum  : &W%-7d &c     | &cStr: &W%-3d  &c|  &cClass: &W%-10s  &c|  &cNumAtt: &W%-2d       &c|\r\n",
	...
	pager_printf_color( ch,"&c|  &cRoom  : &W%-7d    &c  | &cCon: &W%-3d  &c|  &cLevel: &W%-2d          &c|  &cEst   : &W%-4d     &c|\r\n",
			victim->in_room == NULL ? 0 : victim->in_room->vnum,
			get_curr_con(victim), victim->level, victim->emotional_state );
	pager_printf_color( ch,"&c|  &cCount : &W%-7d     &c | &cInt: &W%-3d  &c|  &cHitpt: &W%-5d/%-5d &c|  &cItems : &W%-4d/%-4d&c|\r\n",
			   IS_NPC( victim ) ? victim->pIndexData->count : 1,
			   get_curr_int(victim), victim->hit, victim->max_hit,
			   victim->carry_number, can_carry_n(victim));
	pager_printf_color( ch,"&c|  &cKilled: &W%-7d &c     | &cWis: &W%-3d  &c|  &cMana : &W%-5d/%-5d &c|  &cWeight: &W%-4d/%-4d&c|\r\n",
			IS_NPC( victim ) ? victim->pIndexData->killed : victim->pcdata->mdeaths + victim->pcdata->pdeaths,
			get_curr_wis(victim), victim->mana, victim->max_mana,
			 victim->carry_weight, can_carry_w( victim ));
	pager_printf_color( ch,"&c|  &cHitDie: &W%-3dd%-3d+%-5d&c| &cCha: &W%-3d  &c|  &cMove : &W%-5d/%-5d &c|                   &c|\r\n",
			victim->pIndexData->hitnodice, victim->pIndexData->hitsizedice, victim->pIndexData->hitplus,
			get_curr_cha(victim), victim->move, victim->max_move);
	pager_printf_color( ch,"&c|  &cDamDie: &W%-3dd%-3d+%-5d&c| &cLck: &W%-3d  &c|  &cAC   : &W%-5d       &c|  &cSecs  : &W%-4d     &c|\r\n",
			victim->barenumdie, victim->baresizedie, victim->damplus,
			get_curr_lck(victim), GET_AC( victim ), ( int )victim->played);
	pager_printf_color( ch,"&c|  &cIndam : &W%-3dd%-3d+%-5d&c| &cHit: &W%-3d  &c|  &cAlign: &W%-5d       &c|  &cTimer : &W%-4d     &c|\r\n",
	...
	pager_printf_color( ch,"&c|    &cFighting: &W%-10s     &cMaster : &W%-10s      &cLeader : &W%-10s    &c|\r\n",
                       victim->fighting ? victim->fighting->who->name : "(none)",
                       victim->master ? victim->master->name : "(none)", victim->leader ? victim->leader->name : "(none)" );
	pager_printf_color( ch,"&c|    &cHating  : &W%-10s     &cHunting: &W%-10s      &cFearing: &W%-10s    &c|\r\n",
	...
	pager_printf_color( ch,"&c|&cAct Flags: &W%-66s&c|\r\n",  ext_flag_string( &victim->act, act_flags ) );
	pager_printf_color( ch,"&c|&cAff By: &W%-69s&c|\r\n",  affect_bit_name( &victim->affected_by ) );
	pager_printf_color( ch,"&c|&cAttacks : &W%-67s&c|\r\n",ext_flag_string( &victim->attacks, attack_flags ) );
	pager_printf_color( ch,"&c|&cDefenses: &W%-67s&c|\r\n",ext_flag_string( &victim->defenses, defense_flags ) );
	pager_printf_color( ch,"&c|&cParts: &W%-70s&c|\r\n",
			 flag_string( victim->xflags, part_flags ) );
	pager_printf_color( ch,"&c|&cShort: &W%-70s&c|\r\n",  
			victim->short_descr[0] != '\0' ? victim->short_descr : "(none set)");
	pager_printf_color( ch,"&c|&cLong : &W%s", victim->long_descr);
	pager_printf_color( ch,"&c|&cLanguages: &W"); 
			   for( x = 0; lang_array[x] != LANG_UNKNOWN; x++ )
      if( knows_language( victim, lang_array[x], victim ) || ( IS_NPC( victim ) && victim->speaks == 0 ) )
      {
         if( IS_SET( lang_array[x], victim->speaking ) || ( IS_NPC( victim ) && !victim->speaking ) )
   ...
   for( obj = first_object; obj; obj = obj->next )
   {
      if( obj->in_room && obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC && !str_cmp( buf2, obj->short_descr ) )
      {
         ...
         pager_printf_color( ch, "&P%s:  %s%-12.12s %s  &PIn:  &w%-22.22s  &P[&w%5d&P]   &PTimer:  %s%2d",
                             ...
                             bag ? "&Y" : "&w",
                             capitalize( arg1 ),
                             IS_OBJ_STAT( obj, ITEM_CLANCORPSE ) ? "PK" : "  ",
                             obj->in_room->area->name,
                             obj->in_room->vnum,
                             obj->timer < 1 ? "&w" : obj->timer < 5 ? "" : obj->timer < 10 ? "&Y" : "&w", obj->timer );
         if( bag )
         {
            obj_from_room( obj );
...
void do_nohomepage( CHAR_DATA *ch, const char *argument )
{
   ...
   set_char_color( AT_IMMORT, victim );
   if( IS_SET( victim->pcdata->flags, PCFLAG_NOCPOSE ) )
   {
      REMOVE_BIT( victim->pcdata->flags, PCFLAG_NOCPOSE );
      send_to_char( "You can set your own cpose again.\r\n", victim );
      ch_printf( ch, "NOCPOSE removed from %s.\r\n", victim->name );
   }
   else
   {
      SET_BIT( victim->pcdata->flags, PCFLAG_NOCPOSE );
      STRFREE( victim->pcdata->cpose );
      victim->pcdata->cpose = STRALLOC( "" );
      if( !victim->desc )
         add_loginmsg( victim->name, 12, NULL );
      else
         send_to_char( "You can't set your own cpose!\r\n", victim );
      ch_printf( ch, "NOCPOSE set on %s.\r\n", victim->name );
   }
   save_char_obj( victim );
   return;
}
