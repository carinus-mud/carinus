...
void do_rpgscore( CHAR_DATA* ch, const char* argument )
{
	int start;
...
send_to_pager( "&W----------------------------------------------------------------------------\r\n", ch );
pager_printf( ch, "&YType a command to open menu.	    &W%s - Level %d %s\r\n\r\n", 
	ch->name, ch->level, ch->pcdata->customclass );
pager_printf( ch, "&CSTATUS&W                                  Health: %d/%d\r\n",  ch->hit, ch->max_hit );

send_to_pager( "&CINVENTORY&W                          ",ch);
send_to_char("[",ch);
float value = 25.0 * ((float) ch->hit / (float) ch->max_hit) ;
                          for (start = 1; start <= 25; start++ )      
