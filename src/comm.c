...
void cleanup_memory( void )
{
    ...
    * Cpose table 
    */
   fprintf( stdout, "%s", "Cpose table.\n" );
   for( hash = 0; hash < MAX_CLASS; hash++ )
   {
      for( loopa = 0; loopa < MAX_LEVEL + 1; loopa++ )
      {
         DISPOSE( cpose_table[hash][loopa][0] );
         DISPOSE( cpose_table[hash][loopa][1] );
      }
   }
...
static void SegVio()
{
  ...
  {
    log_printf( "W%cPC: %-20s room: %d", IS_NPC(ch) ? 'N' : ' ', ch->name, ch->in_room->vnum );
  }
  exit(0);
}
*/
...
bool flush_buffer( DESCRIPTOR_DATA * d, bool fPrompt )
{
   ...
   if( IS_SET( ch->pcdata->flags, PCFLAG_BUILDWALK ) )
        send_to_char("&G------(YOU ARE NOW BUILDWALKING)------&D", ch);
	else
...
void nanny_get_name( DESCRIPTOR_DATA * d, char *argument )
{
   ...
   {
      write_to_buffer( d, "\r\nPlease enter your character's name or type NEW.\r\nName: ", 0 );
...
/* hometowns by xander */
	...
	if (ch->race == 0)
   ch->pcdata->hometown = str_dup( "Kroridge" );
else
	if (ch->race == 1)
   ch->pcdata->hometown = str_dup( "O'ran Thalore" );
else
	if (ch->race == 2)
   ch->pcdata->hometown = str_dup( "Kimlodar" );
else
	if (ch->race == 3)
   ch->pcdata->hometown = str_dup( "Shireton" );
else
	if (ch->race == 9)
   ch->pcdata->hometown = str_dup( "BeachCastle" );
else
	if (ch->race == 10)
   ch->pcdata->hometown = str_dup( "Calmarnock" );
else
	if (ch->race == 11)
   ch->pcdata->hometown = str_dup( "Tarnstead" );
else
	if (ch->race == 15)
   ch->pcdata->hometown = str_dup( "Dhubrokh" );
...
char *default_fprompt( CHAR_DATA * ch )
{
   ...
   mudstrlcpy( buf, "&W<&G%h&W/&g%H &WHP ", 60 );
   if( IS_VAMPIRE( ch ) )
      mudstrlcat( buf, "&R%bbp", 60 );
   else
      mudstrlcat( buf, "&C%m&W/&c%M&W M ", 60 );
   mudstrlcat( buf, "&P%v&W/&p%V&W S >", 60 );
...
char *default_prompt( CHAR_DATA * ch )
{
   ...
   if( IS_VAMPIRE( ch ) )
      ...
      mudstrlcat( buf, "&C%m&W/&c%M&W M ", 60 );
   mudstrlcat( buf, "&P%v&W/&p%V&W S &W>", 60 );
   if( IS_NPC( ch ) || IS_IMMORTAL( ch ) )
      mudstrlcat( buf, "%i%R", 60 );
   return buf;
}
