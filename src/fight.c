...
//Thanks to Nick Gammon for cleaning this up for me
 OBJ_DATA *obj = get_eq_char( ch, WEAR_WIELD );
   ...
   switch ( victim->position )
   {
      ...
      pager_printf (ch, "&WYou have earned %d AP from your victim!\r\n", (victim->level));
...
void new_dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, unsigned int dt, OBJ_DATA * obj )
{
   ...
   if( dt == TYPE_HIT )
   {
      ...
      snprintf( buf2, 256, "You %s $N%c &W-(%d)-", vs, punct, dam );
      snprintf( buf3, 256, "$n %s you%c &R-(%d)-", vp, punct, dam );
   }
   else if( dt > TYPE_HIT && is_wielding_poisoned( ch ) )
   {
      ...
      snprintf( buf3, 256, "$n's poisoned %s %s you%c &R-(%d)-", attack, vp, punct, dam );
   }
   else
   {
      if( skill )
