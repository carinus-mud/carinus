...
/*
   ...
   fprintf(fp,"%i",ch->fast_travel_locs[i]);
   if(i != MAX_FAST_TRAVEL_LOCATIONS-1) {fprintf(fp," ");}
...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover )
{
   ...
   if( !found )
   {
      ...
      ch->pcdata->customclass = STRALLOC( "" );
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            ...
            {
               line = fread_line( fp );
               x1 = x2 = x3 = x4 = x5 = x6 = x7 = x8 = x9 = x10 = x11 = 13;
               sscanf( line, "%d %d %d %d %d %d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9, &x10, &x11 );
               ch->mod_str = x1;
               ch->mod_int = x2;
               ch->mod_wis = x3;
               ch->mod_dex = x4;
            ...
            KEY( "Ap", ch->pcdata->ap, fread_number( fp ) );
         ...
         case 'C':
            ...
            if( !strcmp( word, "Cpose" ) )
            {
               ch->pcdata->cpose = fread_string( fp );
               if( isalpha( ch->pcdata->cpose[0] ) || isdigit( ch->pcdata->cpose[0] ) )
               {
                  snprintf( buf, MAX_STRING_LENGTH, " %s", ch->pcdata->cpose );
                  if( ch->pcdata->cpose )
                     STRFREE( ch->pcdata->cpose );
...
/*
            if( !strcmp(word, "FastTravelLocs"))
            {
               ...
               for( x = 0; x < MAX_FAST_TRAVEL_LOCATIONS; x++ )
                  ch->fast_travel_locs[x] = fread_number( fp );
               fread_to_eol( fp );
               fMatch = TRUE;
               break;
...
*/
         ...
         case 'H':
            ...
            {
               ch->pcdata->hometown = fread_string( fp );
               if( isalpha( ch->pcdata->hometown[0] ) || isdigit( ch->pcdata->hometown[0] ) )
               {
                  snprintf( buf, MAX_STRING_LENGTH, " %s", ch->pcdata->hometown );
                  if( ch->pcdata->hometown )
                     STRFREE( ch->pcdata->hometown );
                  ch->pcdata->hometown = STRALLOC( buf );
               }
               fMatch = TRUE;
               break;
            }
	   ...
	   if( !strcmp( word, "Ignored" ) )
         ...
         case 'E':
            if( !strcmp( word, "End" ) )
            {
               ...
               if( !ch->pcdata->title )
                  ch->pcdata->title = STRALLOC( "" );
               if( !ch->pcdata->customclass )
                  ch->pcdata->customclass = STRALLOC( "" );
               if( !ch->pcdata->cpose )
                  ch->pcdata->cpose = STRALLOC( "" );
               if( !ch->pcdata->hometown )
                  ch->pcdata->hometown = STRALLOC( "" );
...
void fread_obj( CHAR_DATA * ch, FILE * fp, short os_type )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'T':
            KEY( "Tier", obj->tier, fread_number( fp ) );
