...
const char *cpose_table[MAX_CLASS][MAX_LEVEL + 1][2];
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'C':
            ...
            if( !str_cmp( word, "cpose" ) )
            {
               ...
               {
                  cpose_table[cl][tlev][0] = fread_string_nohash( fp );
                  cpose_table[cl][tlev][1] = fread_string_nohash( fp );
                  ++tlev;
               }
               else
                  bug( "%s: Too many cposes", __func__ );
               fMatch = TRUE;
               break;
            }
