...
//  fclose(fp);
  ...
  fprintf(fp, "<head>\n");
  fprintf(fp, "<title>");
  fprintf(fp, "Players currently on Your Mud's Name Here");
  ...
  fprintf(fp, "<h1><center>Currently Online:</center></h1>\n\n");
  ...
  for ( d = first_descriptor; d != NULL ; d = d->next )
  {
    ...
    {
	fprintf(fp, "<TR ALIGN=CENTER VALIGN=CENTER>\n\r");
	fprintf(fp, "[%s] Level %d -  %s, %s\n\r<br />", wch->pcdata->customclass, wch->level, wch->name, wch->pcdata->title);
